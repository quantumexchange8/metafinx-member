...
<template>
  <div class="flex flex-nowrap md:grid md:grid-cols-4 gap-3 overflow-x-auto md:overflow-visible my-8">
    <div class="flex-1 rounded-[10px] border border-gray-200 shadow dark:border-transparent dark:bg-gray-700">
      <div class="px-5 py-2.5 flex flex-col justify-between">
        <p class="text-gray-400 text-xs md:text-sm w-32 md:w-full">{{ $t('public.affiliate.total_VAD') }}</p>
    ...
    <div class="flex-1 rounded-[10px] border border-gray-200 shadow dark:border-transparent dark:bg-gray-700">
      <div class="px-5 py-2.5 flex flex-col justify-between">
        <p class="text-gray-400 text-xs md:text-sm w-32 md:w-full">{{ $t('public.affiliate.total_direct_referrals') }}</p>
        <p class="text-gray-800 dark:text-white text-xl font-semibold">{{ props.referredCounts }}</p>
      </div>
    </div>
    <div class="flex-1 rounded-[10px] border border-gray-200 shadow dark:border-transparent dark:bg-gray-700">
      <div class="px-5 py-2.5 flex flex-col justify-between">
        <p class="text-gray-400 text-xs md:text-sm w-32 md:w-full">{{ $t('public.affiliate.total_affiliates') }}</p>
        <p class="text-gray-800 dark:text-white text-xl font-semibold">{{ props.totalAffiliate }}</p>
      </div>
    </div>
    <div class="flex-1 rounded-[10px] border border-gray-200 shadow dark:border-transparent dark:bg-gray-700">
      <div class="px-5 py-2.5 flex flex-col justify-between">
        <p class="text-gray-400 text-xs md:text-sm w-32 md:w-full">{{ $t('public.affiliate.total_generation') }}</p>
        <p class="text-gray-800 dark:text-white text-xl font-semibold">{{ props.totalGeneration }}</p>
      </div>
    </div>
  </div>

  <div class="p-5 rounded-[20px] bg-gray-700 mb-5">
    ...
    <div class="flex gap-2 items-center">
      ...
      <div class="w-full">
        <vue-tailwind-datepicker :placeholder="$t('public.report.date_picker_placeholder')" :formatter="formatter"
          separator=" - " as-single use-range :shortcuts="false" v-model="date"
      ...
      <div class="w-full">
        <BaseListbox v-model="rank" :options="rankFilter">
        </BaseListbox>
      </div>
      <div class="w-full">
        <BaseListbox v-model="level" :options="levelFilter">
        </BaseListbox>
      </div>
      <div>
        <Button type="button" variant="secondary" @click="clearFilter">
          Clear
        </Button>
      </div>
    </div>
  ...
  <div class="flex justify-center mt-4" v-if="!isLoading">
    <TailwindPagination :item-classes=paginationClass :active-classes=paginationActiveClass :data="referralTableData"
